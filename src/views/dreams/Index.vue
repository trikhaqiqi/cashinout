                ...
                <div class="mb-5">
                    <div class="border rounded-lg overflow-hidden">
                        ...
                        <div class="h-96 overflow-y-scroll">
                            <template v-for="(dream, index) in dreams" :key="index">
                                <router-link :to="`/dreams/${dream.id}`" href="#" class="px-6 py-5 flex justify-between items-center hover:bg-gray-50 border-b mb-2">
                                    <span class="flex flex-col">
                                        <span class="text-gray-500 text-xs">status</span>
                                        <span>{{ dream.name }}</span>
                                    </span>
                                    <span class="text-gray-500 text-xs">{{ dream.when }}</span>
                                </router-link>
                            </template>
                        </div>
                    </div>
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            .then(response => {
              ...
              //assign state dreams with response data
              dreams.value = response.data.data
        ...
        const add = async () => {
            let response = await axios.post('api/dream', form)
            dreams.value.dreams.unshift(response.data.dream)
        }
